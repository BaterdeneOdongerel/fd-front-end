...
import { fetchOwnerOrders} from './../Utils';
import {TableRow, OrderDetails} from './../common-component';

export function OwnerOrder(props) {
  ...
  const loadMore = () => {
    if (loading === false && loadMore){
      setLoading(true);
      fetchOwnerOrders(setLoading, setCurrentData, setError, count, perPage);
    }
  }
  ...
  return (
    <div className="content">
     ...
     { detailData && <OrderDetails orderList={data} setOrderList={setData} data={detailData} isOwner={true} handleClose={() => setDetailData(null)} ></OrderDetails> }
      <div className="ml-5">
